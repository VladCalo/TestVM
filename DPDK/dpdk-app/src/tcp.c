...
void tcp_tx_loop(uint16_t port_id, struct rte_mempool *mbuf_pool) {
    ...
    while (1) {
        if (state == 0) {
            ...
            const size_t pkt_size = sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_tcp_hdr);
            ...
            eth_hdr->src_addr.addr_bytes[1] = 0x11;
            eth_hdr->src_addr.addr_bytes[2] = 0x22;
            eth_hdr->src_addr.addr_bytes[3] = 0x33;
            eth_hdr->src_addr.addr_bytes[4] = 0x44;
            eth_hdr->src_addr.addr_bytes[5] = 0x55;

            eth_hdr->dst_addr.addr_bytes[0] = 0x52;
            eth_hdr->dst_addr.addr_bytes[1] = 0x54;
            eth_hdr->dst_addr.addr_bytes[2] = 0x00;
            eth_hdr->dst_addr.addr_bytes[3] = 0xa2;
            eth_hdr->dst_addr.addr_bytes[4] = 0x6c;
            eth_hdr->dst_addr.addr_bytes[5] = 0xb6;
            ...
            tcp_hdr->src_port = rte_cpu_to_be_16(TCP_SRC_PORT);
            tcp_hdr->dst_port = rte_cpu_to_be_16(TCP_DST_PORT);
            tcp_hdr->sent_seq = rte_cpu_to_be_32(tx_seq);
            tcp_hdr->recv_ack = 0;
        ...
        if (state == 1) {
            ...
            for (int i = 0; i < nb_rx; i++) {
                ...
                if ((tcp->tcp_flags & (RTE_TCP_SYN_FLAG | RTE_TCP_ACK_FLAG)) == (RTE_TCP_SYN_FLAG | RTE_TCP_ACK_FLAG)) {
                    ...
                    memcpy(pkt_data, eth, sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_tcp_hdr));
                    struct rte_ether_hdr *ack_eth = (struct rte_ether_hdr *)pkt_data;
                    struct rte_ipv4_hdr *ack_ip = (struct rte_ipv4_hdr *)(ack_eth + 1);
                    struct rte_tcp_hdr *ack_tcp = (struct rte_tcp_hdr *)(ack_ip + 1);

                    ack_tcp->tcp_flags = RTE_TCP_ACK_FLAG;
                    ack_tcp->sent_seq = rte_cpu_to_be_32(tx_seq + 1);
                    ack_tcp->recv_ack = rte_cpu_to_be_32(rx_ack);
                    ack_tcp->cksum = 0;
    ...
    while (1) {
        ...
        for (int i = 0; i < nb_rx; i++) {
            ...
            if (tcp->tcp_flags & RTE_TCP_SYN_FLAG && !(tcp->tcp_flags & RTE_TCP_ACK_FLAG)) {
                ...
                ack_eth->dst_addr = eth->src_addr;
                ack_eth->src_addr = eth->dst_addr;

                ack_ip->src_addr = ip->dst_addr;
                ack_ip->dst_addr = ip->src_addr;
                ack_ip->hdr_checksum = 0;
                ack_ip->hdr_checksum = checksum(ack_ip, sizeof(struct rte_ipv4_hdr));

                ack_tcp->sent_seq = rte_cpu_to_be_32(2000);
                ack_tcp->recv_ack = rte_cpu_to_be_32(rte_be_to_cpu_32(tcp->sent_seq) + 1);
                ack_tcp->tcp_flags = RTE_TCP_SYN_FLAG | RTE_TCP_ACK_FLAG;
                ack_tcp->cksum = 0;

                rte_eth_tx_burst(port_id, 0, &syn_ack, 1);
                printf("RX: Sent SYN-ACK\n");
            } else if (tcp->tcp_flags & RTE_TCP_ACK_FLAG) {
